"""
Train the base encoder on TACRED.
"""
...
for epoch in range(1, opt['num_epoch'] + 1):
    ...
    for _, batch in enumerate(dev_batch):
        preds, _, loss = base_model.predict(batch)
        predictions += preds
        dev_loss += loss
    ...
    base_model.save(model_file, epoch)
    if epoch == 1 or dev_f1 > max(dev_f1_history):
        copyfile(model_file, model_save_dir + '/best_model.pt')
        print("new best model saved.")
        file_logger.log("new best model saved at epoch {}: {:.2f}\t{:.2f}\t{:.2f}".format(epoch, dev_p*100, dev_r*100, dev_f1*100))
    ...
    if len(dev_f1_history) > opt['decay_epoch'] and dev_f1 <= dev_f1_history[-1] and \
        ...
        base_model.update_lr(current_lr)
