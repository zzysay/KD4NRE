...
class GCNRelationModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        gcn_output, pool_mask, input_vectors = self.gcn(adj, inputs)

        # pooling
        subj_mask, obj_mask = subj_pos.eq(0).eq(0).unsqueeze(2), obj_pos.eq(0).eq(0).unsqueeze(2)  # invert mask
        pool_type = self.opt['pooling']
        pooling_output = pool(gcn_output, pool_mask, type=pool_type)
        subj_out = pool(gcn_output, subj_mask, type=pool_type)
        obj_out = pool(gcn_output, obj_mask, type=pool_type)
        outputs = torch.cat([pooling_output, subj_out, obj_out], dim=1)
        final_hidden = self.out_mlp(outputs)

        # classifier
        logits = self.classifier(final_hidden)

        loss_part = [logits, pooling_output]
        aux_part = [final_hidden, input_vectors, gcn_output]
...
class GCN(nn.Module):
    ...
    def forward(self, adj, inputs):
        ...
        input_vectors = torch.cat(embs, dim=2)
        embs = self.in_drop(input_vectors)
        ...
        return gcn_inputs, mask, input_vectors
