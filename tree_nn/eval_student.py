...
for i, b in enumerate(batch):
    final_preds, inst_preds, aux_preds, final_probs, inst_probs, aux_probs = \
        student_model.predict_all(b)
...
if len(args.out) > 0:
    ...
    with open(outfile, 'w') as fw:
        for f_prob, i_prob, m_prob in zip(all_final_probs, all_inst_probs, all_aux_probs):
            fw.write(json.dumps([round(p, 4) for p in f_prob]))
            fw.write('\r\n')
            fw.write(json.dumps([round(p, 4) for p in i_prob]))
            fw.write('\r\n')
            fw.write(json.dumps([round(p, 4) for p in m_prob]))
            fw.write('\r\n')
            fw.write('\r\n')
